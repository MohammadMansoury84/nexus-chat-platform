...
class Bounds:
    ...
    def __str__(self):
        ...
        if self.is_delegation:
            ...
            zonecut = ""
        return (
            f"{self.left} {op} {self.name} < {self.right}{zonecut}; {self.closest_encloser}"
        )
...
class ImmutableVersion(dns.zone.Version):
    def __init__(self, version: dns.zone.Version):
        if not isinstance(version, WritableVersion):
            raise ValueError(
                "a dns.btreezone.ImmutableVersion requires a dns.btreezone.WritableVersion"
            )
...
class Zone(dns.versioned.Zone):
    ...
    writable_version_factory: Callable[[dns.zone.Zone, bool], dns.zone.Version] | None = (
        WritableVersion
    )
