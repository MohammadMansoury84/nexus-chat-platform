...
def split_email(email: str) -> tuple[str | None, str, str, bool]:
    ...
    domain_part = domain_part.removeprefix("@")
...
def validate_email_local_part(
    ...
    # restrictions on the placement of dots, as in dot-atom local parts.
        ...
        bad_chars = {safe_character_display(c) for c in local if not QTEXT_INTL.match(c)}
        if bad_chars:
            raise EmailSyntaxError(
    ...
    bad_chars = {safe_character_display(c) for c in local if not ATEXT_INTL_DOT_RE.match(c)}
    if bad_chars:
        raise EmailSyntaxError(
...
def uts46_valid_char(char: str) -> bool:
    ...
    if (
        ...
        # tests below.
        return True
    if unicodedata.category(chr(c)) in ("Cf", "Cn", "Co", "Cs", "Zs", "Zl", "Zp"):
        ...
        return False
    if "002E" in unicodedata.decomposition(chr(c)).split(" "):
...
def validate_email_domain_name(
    ...
    for label in domain.split("."):
        if re.match(r"(?!xn)..--", label, re.IGNORECASE):
            raise EmailSyntaxError(
    ...
    # case for this.
    bad_chars = {
        safe_character_display(c) for c in domain_i18n if not ATEXT_HOSTNAME_INTL.match(c)
    }
    if bad_chars:
        raise EmailSyntaxError(
            "The part after the @-sign contains invalid characters: "
