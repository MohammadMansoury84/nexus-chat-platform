...
class _Pipeline(Generic[_InT, _OutT]):
    ...
    @overload
    def validate_as(
        self, tp: type[_NewOutT], *, strict: bool = False
    ) -> _Pipeline[_InT, _NewOutT]: ...
    ...
    def validate_as(
        self, tp: type[_NewOutT] | EllipsisType | Any, *, strict: bool = False
    ) -> _Pipeline[_InT, Any]:  # type: ignore
...
def _apply_step(
    step: _Step, s: cs.CoreSchema | None, handler: GetCoreSchemaHandler, source_type: Any
) -> cs.CoreSchema:
...
def _apply_constraint(  # noqa: C901
    ...
    if isinstance(constraint, annotated_types.Gt):
        ...
        if s and s["type"] in {"int", "float", "decimal"}:
            s = s.copy()
            if (
                (s["type"] == "int" and isinstance(gt, int))
                or (s["type"] == "float" and isinstance(gt, float))
                or (s["type"] == "decimal" and isinstance(gt, Decimal))
            ):
                s["gt"] = gt
        else:
        ...
        if s and s["type"] in {"int", "float", "decimal"}:
            s = s.copy()
            if (
                (s["type"] == "int" and isinstance(ge, int))
                or (s["type"] == "float" and isinstance(ge, float))
                or (s["type"] == "decimal" and isinstance(ge, Decimal))
            ):
                s["ge"] = ge
        ...
        if s and s["type"] in {"int", "float", "decimal"}:
            s = s.copy()
            if (
                (s["type"] == "int" and isinstance(lt, int))
                or (s["type"] == "float" and isinstance(lt, float))
                or (s["type"] == "decimal" and isinstance(lt, Decimal))
            ):
                s["lt"] = lt
        ...
        if s and s["type"] in {"int", "float", "decimal"}:
            s = s.copy()
            if (
                (s["type"] == "int" and isinstance(le, int))
                or (s["type"] == "float" and isinstance(le, float))
                or (s["type"] == "decimal" and isinstance(le, Decimal))
            ):
                s["le"] = le
        ...
        if s and s["type"] in {"int", "float", "decimal"}:
            s = s.copy()
            if (
                (s["type"] == "int" and isinstance(multiple_of, int))
                or (s["type"] == "float" and isinstance(multiple_of, float))
                or (s["type"] == "decimal" and isinstance(multiple_of, Decimal))
            ):
