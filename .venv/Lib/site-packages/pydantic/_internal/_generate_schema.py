...
def apply_each_item_validators(
    ...
    if schema["type"] == "nullable":
        schema["schema"] = apply_each_item_validators(
            schema["schema"], each_item_validators
        )
        return schema
...
class GenerateSchema:
    ...
    def _enum_schema(self, enum_type: type[Enum]) -> CoreSchema:
        ...
        if cases:
            ...
            default_missing = (
                getattr(enum_type._missing_, "__func__", None) is Enum._missing_.__func__
        ...
        def get_json_schema_no_cases(_, handler: GetJsonSchemaHandler) -> JsonSchemaValue:
            json_schema = handler(
                core_schema.enum_schema(enum_type, cases, sub_type=sub_type, ref=enum_ref)
            )
            original_schema = handler.resolve_ref_schema(json_schema)
            original_schema.update(js_updates)
            return json_schema
    ...
    def _model_schema(self, cls: type[BaseModel]) -> core_schema.CoreSchema:
        ...
        with self.defs.get_schema_or_ref(cls) as (model_ref, maybe_schema):
            ...
            with (
                ...
                if (
                    core_config.get("extra_fields_behavior") == "allow"
                ...
                if cls.__pydantic_root_model__:
                    inner_schema, metadata = self._common_field_schema(
                        "root", fields["root"], decorators
                    )
                    if cls.__doc__ and metadata.get("pydantic_js_updates", {}).get(
                        "description"
                    ):
                    ...
                    inner_schema = apply_validators(
                        fields_schema, decorators.root_validators.values()
                    )
                    inner_schema = apply_model_validators(
                        inner_schema, model_validators, "inner"
                    )
    ...
    def match_type(self, obj: Any) -> core_schema.CoreSchema:  # noqa: C901
        ...
        if typing_objects.is_any(obj) or obj is object:
            return core_schema.any_schema()
        ...
        if obj is MultiHostUrl:
            return core_schema.multi_host_url_schema()
        if obj is None or obj is _typing_extra.NoneType:
        ...
        if obj in FROZEN_SET_TYPES:
            return self._frozenset_schema(Any)
        if obj in SEQUENCE_TYPES:
            return self._sequence_schema(Any)
        if obj in ITERABLE_TYPES:
            return self._iterable_schema(obj)
        if obj in DICT_TYPES:
            return self._dict_schema(Any, Any)
        if obj in PATH_TYPES:
            return self._path_schema(obj, Any)
        if obj in DEQUE_TYPES:
            return self._deque_schema(Any)
        if obj in MAPPING_TYPES:
            return self._mapping_schema(obj, Any, Any)
        if obj in COUNTER_TYPES:
            return self._mapping_schema(obj, Any, int)
        if typing_objects.is_typealiastype(obj):
            return self._type_alias_type_schema(obj)
        if obj is type:
            return self._type_schema()
        if _typing_extra.is_callable(obj):
            return core_schema.callable_schema()
        if typing_objects.is_literal(get_origin(obj)):
        ...
        if _typing_extra.is_namedtuple(obj):
            return self._namedtuple_schema(obj, None)
        if typing_objects.is_newtype(obj):
            # NewType, can't use isinstance because it fails <3.10
            return self.generate_schema(obj.__supertype__)
        if obj in PATTERN_TYPES:
            return self._pattern_schema(obj)
        if _typing_extra.is_hashable(obj):
        ...
        if isinstance(obj, VALIDATE_CALL_SUPPORTED_TYPES):
    ...
    def _literal_schema(self, literal_type: Any) -> CoreSchema:
        ...
        expected = list(
            get_literal_values(literal_type, type_check=False, unpack_type_aliases="eager")
        )
    ...
    def _typed_dict_schema(
        ...
        with (
            ...
            with self._config_wrapper_stack.push(config):
                ...
                decorators = DecoratorInfos.build(
                    typed_dict_cls, replace_wrapped_methods=False
                )
                ...
                extra_items = getattr(
                    typed_dict_cls, "__extra_items__", typing_extensions.NoExtraItems
                )
                if is_closed:
                    ...
                    extras_schema = self.generate_schema(
                ...
                if (config_extra := self._config_wrapper.extra) in ("allow", "forbid"):
                    if is_closed and config_extra == "allow":
                        ...
                        not typing_objects.is_noextraitems(extra_items)
                        and config_extra == "forbid"
                    ):
                        warnings.warn(
    ...
    def _namedtuple_schema(
        ...
        with (
            ...
            annotations = {
                field_name: annotations.get(field_name, Any)
    ...
    def _subclass_schema(self, type_: Any) -> core_schema.CoreSchema:
        ...
        if typing_objects.is_any(type_param):
            return self._type_schema()
        if typing_objects.is_typealiastype(type_param):
            return self.generate_schema(type[type_param.__value__])
        if typing_objects.is_typevar(type_param):
            ...
            if type_param.__constraints__:
                return core_schema.union_schema(
                    [self.generate_schema(type[c]) for c in type_param.__constraints__]
                )
            return self._type_schema()
        if is_union_origin(get_origin(type_param)):
    ...
    def _pattern_schema(self, pattern_type: Any) -> core_schema.CoreSchema:
        ...
        metadata = {
            "pydantic_js_functions": [lambda _1, _2: {"type": "string", "format": "regex"}]
        }
        ...
        with (
            ...
            with self._ns_resolver.push(dataclass), self._config_wrapper_stack.push(config):
                ...
                if decorators is None:
                    decorators = DecoratorInfos.build(
                        dataclass, replace_wrapped_methods=False
                    )
                    decorators.update_from_config(self._config_wrapper)
                ...
                inner_schema = apply_validators(
                    args_schema, decorators.root_validators.values()
                )
    ...
    def _arguments_v3_schema(
        ...
        mode_lookup: dict[
            _ParameterKind,
            Literal["positional_only", "positional_or_keyword", "var_args", "keyword_only"],
        ] = {
            Parameter.POSITIONAL_ONLY: "positional_only",
            Parameter.POSITIONAL_OR_KEYWORD: "positional_or_keyword",
    ...
    def _computed_field_schema(
        ...
        update_core_metadata(
            core_metadata,
            pydantic_js_updates={"readOnly": True, **(pydantic_js_updates or {})},
            pydantic_js_extra=pydantic_js_extra,
        )
    ...
    def _apply_single_annotation(
        ...
        if isinstance(metadata, FieldInfo):
            if (
                ...
                for attr, value in (
                    unsupported_attributes := self._get_unsupported_field_info_attributes(
                        metadata
                    )
                ):
                    warnings.warn(
        ...
        if schema["type"] == "union" and any(
            ...
            filtered_choices = [
                ...
                if (choice[0] if isinstance(choice, tuple) else choice)["type"]
                != "missing-sentinel"
            ]
            ...
            if len(filtered_choices) == 1:
                ...
                inner = (
                    filtered_choices[0][0]
                    if isinstance(filtered_choices[0], tuple)
                    else filtered_choices[0]
                )
                inner = self._apply_single_annotation(inner, metadata)
                ...
                new_union["choices"] = [
                    ...
                    and choice["type"] != "missing-sentinel"
                    else choice
                    for choice in schema["choices"]
                ]
                return new_union
    ...
    def _get_unsupported_field_info_attributes(
        ...
        for (
            unused_metadata_name,
            unset_value,
        ) in UNSUPPORTED_STANDALONE_FIELDINFO_ATTRIBUTES:
            if (
                (unused_metadata_value := getattr(field_info, unused_metadata_name))
                is not unset_value
...
_VALIDATOR_F_MATCH: Mapping[
    ...
    ("before", "no-info"): lambda f, schema: core_schema.no_info_before_validator_function(
        f, schema
    ),
    ("after", "no-info"): lambda f, schema: core_schema.no_info_after_validator_function(
        f, schema
    ),
    ("plain", "no-info"): lambda f, _: core_schema.no_info_plain_validator_function(f),
    ("wrap", "no-info"): lambda f, schema: core_schema.no_info_wrap_validator_function(
        f, schema
    ),
    ("before", "with-info"): lambda f, schema: (
        core_schema.with_info_before_validator_function(f, schema)
    ),
    ("after", "with-info"): lambda f, schema: (
        core_schema.with_info_after_validator_function(f, schema)
    ),
    ("plain", "with-info"): lambda f, _: core_schema.with_info_plain_validator_function(f),
    ("wrap", "with-info"): lambda f, schema: core_schema.with_info_wrap_validator_function(
        f, schema
    ),
}
    ...
    def finalize_schema(self, schema: CoreSchema) -> CoreSchema:
        ...
        for ref, inlinable_def_ref in gather_result["collected_references"].items():
            if (
                ...
                and (inlining_behavior := _inlining_behavior(inlinable_def_ref)) != "keep"
            ):
