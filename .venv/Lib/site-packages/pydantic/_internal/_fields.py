    ...
    for protected_namespace in protected_namespaces:
        ...
        if ns_violation:
            ...
            for pn in protected_namespaces:
                ...
                elif not ann_name.startswith(pn):
...
def _apply_field_title_generator_to_field_info(
    ...
    if field_info.title is None:
        ...
        if not isinstance(title, str):
            raise TypeError(
                f"field_title_generator {title_generator} must return str, not {title.__class__}"
            )
...
def update_field_from_config(
    ...
    if field_title_generator is not None:
        _apply_field_title_generator_to_field_info(
            field_title_generator, field_name, field_info
        )
    if config_wrapper.alias_generator is not None:
        _apply_alias_generator_to_field_info(
            config_wrapper.alias_generator, field_name, field_info
        )
    ...
    *,
    ...
    for ann_name, (ann_type, evaluated) in type_hints.items():
        ...
        if assigned_value is not PydanticUndefined and (
            ...
            any(
                getattr(BaseModel_, depr_name, None) is assigned_value
                for depr_name in _deprecated_method_names
            )
        ...
        for base in bases:
            if hasattr(base, ann_name):
                ...
                generic_origin = getattr(cls, "__pydantic_generic_metadata__", {}).get(
                    "origin"
                )
...
def rebuild_model_fields(
    ...
    *,
    ...
    with ns_resolver.push(cls):
        ...
        if (
            cls.__pydantic_extra_info__ is not None
            and not cls.__pydantic_extra_info__.complete
        ):
...
def _recreate_field_info(
    ...
    *,
    ...
    new_field.description = (
        new_field.description if new_field.description is not None else existing_desc
    )
    if not evaluated:
    ...
    for base in reversed(cls.__mro__):
        ...
        with ns_resolver.push(base):
            for ann_name, dataclass_field in dataclass_fields.items():
                ...
                ann_type, evaluated = _typing_extra.try_eval_type(
