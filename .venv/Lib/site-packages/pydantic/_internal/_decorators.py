...
DecoratedType: TypeAlias = "classmethod[Any, Any, ReturnType] | staticmethod[Any, ReturnType] | Callable[..., ReturnType] | property"
    ...
    def build(
        ...
        for base in reversed(mro(typ)[1:-1]):
            ...
            res.validators.update(
                {k: v.bind_to_cls(typ) for k, v in existing.validators.items()}
            )
            res.field_validators.update(
                {k: v.bind_to_cls(typ) for k, v in existing.field_validators.items()}
            )
            res.root_validators.update(
                {k: v.bind_to_cls(typ) for k, v in existing.root_validators.items()}
            )
            res.field_serializers.update(
                {k: v.bind_to_cls(typ) for k, v in existing.field_serializers.items()}
            )
            res.model_serializers.update(
                {k: v.bind_to_cls(typ) for k, v in existing.model_serializers.items()}
            )
            res.model_validators.update(
                {k: v.bind_to_cls(typ) for k, v in existing.model_validators.items()}
            )
            res.computed_fields.update(
                {k: v.bind_to_cls(typ) for k, v in existing.computed_fields.items()}
            )

        decorator_infos, to_replace = _decorator_infos_for_class(
            typ, collect_to_replace=True
        )
        ...
        if isinstance(var_value, PydanticDescriptorProxy):
            ...
            if isinstance(info, ValidatorDecoratorInfo):
                ...
                res.computed_fields[var_name] = Decorator.build(
                    typ, cls_var_name=var_name, shim=None, info=info
                )
...
AnyDecoratorCallable: TypeAlias = "classmethod[Any, Any, Any] | staticmethod[Any, Any] | partialmethod[Any] | Callable[..., Any]"
