...
class _ApplyInferredDiscriminator:
    ...
    * Handling various edge cases such as 'definitions', 'default', 'nullable' schemas, and more
    ...
    def _infer_discriminator_values_for_choice(  # noqa C901
        ...
        if _core_utils.is_function_with_inner_schema(choice):
            return self._infer_discriminator_values_for_choice(
                choice["schema"], source_name=source_name
            )

        if choice["type"] == "lax-or-strict":
            return sorted(
                set(
        ...
        if choice["type"] == "model" or choice["type"] == "dataclass":
            return self._infer_discriminator_values_for_choice(
    ...
    def _infer_discriminator_values_for_inner_schema(
        ...
        if schema["type"] == "model" and schema.get("root_model"):
            # Support RootModel[Literal[...]] as discriminator field type
            return self._infer_discriminator_values_for_inner_schema(
                schema["schema"], source
            )

        if schema["type"] in {"function-before", "function-wrap", "function-plain"}:
