...
class _DefinitionsRemapping:
    ...
    def from_prioritized_choices(
        ...
        for _iter in range(
            ...
            schemas_for_alternatives: dict[DefsRef, list[JsonSchemaValue]] = defaultdict(
                list
            )
...
class GenerateJsonSchema:
    ...
    def _sort_recursive(self, value: Any, parent_key: str | None = None) -> Any:
        ...
        if isinstance(value, list):
            sorted_list: list[JsonSchemaValue] = [
                self._sort_recursive(item, parent_key) for item in value
            ]
            return sorted_list
        return value
    ...
    def decimal_schema(self, schema: core_schema.DecimalSchema) -> JsonSchemaValue:
        ...
        def get_decimal_pattern(schema: core_schema.DecimalSchema) -> str:
            ...
            pattern = r"^(?!^[-+.]*$)[+-]?0*"  # check it is not empty string and not one or sequence of ".+-" characters.
        ...
        json_schema = self.str_schema(
            core_schema.str_schema(pattern=get_decimal_pattern(schema))
        )
    ...
    def missing_sentinel_schema(
        self, schema: core_schema.MissingSentinelSchema
    ) -> JsonSchemaValue:
    ...
    @deprecated(
        "`tuple_variable_schema` is deprecated. Use `tuple_schema` instead.", category=None
    )
    @final
    ...
    def typed_dict_schema(self, schema: core_schema.TypedDictSchema) -> JsonSchemaValue:
        ...
        if (
            "extras_schema" in schema
            and schema["extras_schema"] != core_schema.any_schema()
        ):
    ...
    def _update_class_schema(
        ...
        * deprecated
        ...
        if cls is BaseModel:
            docstring = None
        elif is_stdlib_dataclass(
            cls
        ):  # For Pydantic dataclasses, we already handle this at class creation
        ...
        if (
            "additionalProperties" not in json_schema
        ):  # This check is particularly important for `typed_dict_schema()`
            if extra == "allow":
                json_schema["additionalProperties"] = True
            elif extra == "forbid":
                json_schema["additionalProperties"] = False
        ...
        if isinstance(json_schema_extra, dict):
            ...
            if len(_typing_extra.signature_no_eval(json_schema_extra).parameters) > 1:
                json_schema_extra = cast(
                    "Callable[[JsonDict, type[Any]], None]", json_schema_extra
                )
    ...
    def kw_arguments_schema(
        ...
        for argument in arguments:
            ...
            if "title" not in argument_schema and self.field_title_should_be_set(
                argument["schema"]
            ):
    ...
    def arguments_v3_schema(self, schema: core_schema.ArgumentsV3Schema) -> JsonSchemaValue:
        ...
        for argument in arguments:
            ...
            if (
                ...
                mode
                not in {"var_args", "var_kwargs_uniform", "var_kwargs_unpacked_typed_dict"}
...
def model_json_schema(
    ...
    assert not isinstance(cls.__pydantic_core_schema__, _mock_val_ser.MockCoreSchema), (
...
def models_json_schema(
    models: Sequence[tuple[type[BaseModel | PydanticDataclass], JsonSchemaMode]],
    *,
    by_alias: bool = True,
    title: str | None = None,
    ...
    dict[tuple[type[BaseModel | PydanticDataclass], JsonSchemaMode], JsonSchemaValue],
    JsonSchemaValue,
]:
    ...
    instance = schema_generator(
        by_alias=by_alias, ref_template=ref_template, union_format=union_format
    )
    inputs: list[tuple[type[BaseModel | PydanticDataclass], JsonSchemaMode, CoreSchema]] = [
...
def _get_ser_schema_for_default_value(
    schema: CoreSchema,
) -> core_schema.PlainSerializerFunctionSerSchema | None:
