...
def field_schema(
    ...
    *,
    ...
    ref_prefix: str | None = None,
    ref_template: str = default_ref_template,
    known_models: TypeModelSet | None = None,
...
_str_types_attrs: tuple[tuple[str, type | tuple[type, ...], str], ...] = (
    ("max_length", numeric_types, "maxLength"),
    ("min_length", numeric_types, "minLength"),
    ("regex", str, "pattern"),
)

_numeric_types_attrs: tuple[tuple[str, type | tuple[type, ...], str], ...] = (
    ...
    *,
    ...
    if field.shape in {
        ...
        if regex:
            ...
            f_schema["patternProperties"] = {
                ConstrainedStr._get_pattern(regex): items_schema
            }
        if items_schema:
            # The dict values are not simply Any, so they need a schema
            f_schema["additionalProperties"] = items_schema
    elif field.shape == SHAPE_TUPLE or (
        field.shape == SHAPE_GENERIC and not issubclass(field.type_, BaseModel)
    ):
        sub_schema = []
        ...
        if field.shape == SHAPE_GENERIC:
            all_of_schemas = (
                sub_schema[0]
                if sub_fields_len == 1
                else {"type": "array", "items": sub_schema}
            )
...
def enum_process_schema(
    enum: type[Enum], *, field: ModelField | None = None
) -> dict[str, Any]:
...
def field_singleton_sub_fields_schema(
    ...
    *,
    ...
    if field_has_discriminator:
        ...
        discriminator_models_refs: dict[str, str | dict[str, Any]] = {}
        ...
        for discriminator_value, sub_field in field.sub_fields_mapping.items():
            ...
            if is_union(get_origin(sub_field.type_)):
                ...
                discriminator_model_ref = get_schema_ref(
                    discriminator_model_name, ref_prefix, ref_template, False
                )
                discriminator_models_refs[discriminator_value] = discriminator_model_ref[
                    "$ref"
                ]
...
def get_schema_ref(
    name: str, ref_prefix: str | None, ref_template: str, schema_overrides: bool
) -> dict[str, Any]:
    if ref_prefix:
...
def field_singleton_schema(  # noqa: C901 (ignore complexity)
    ...
    *,
    ...
    if (
        field_type is Any
        or field_type is object
        or field_type.__class__ == TypeVar
        or get_origin(field_type) is type
    ):
    ...
    if is_literal_type(field_type):
        values = tuple(
            x.value if isinstance(x, Enum) else x for x in all_literal_values(field_type)
        )
...
def multitypes_literal_field_for_schema(
    ...
    distinct_literals = (
        Literal[tuple(same_type_values)]
...
def get_annotation_from_field_info(
    ...
    if constraints:
        annotation, used_constraints = get_annotation_with_constraints(
...
) -> tuple[type[Any], set[str]]:  # noqa: C901
    ...
    def go(type_: Any) -> type[Any]:
        if (
            ...
            or lenient_issubclass(
                type_, (ConstrainedList, ConstrainedSet, ConstrainedFrozenSet)
            )
        ):
            return type_
        ...
        if origin is not None:
            ...
            if issubclass(origin, set) and (
                field_info.min_items is not None or field_info.max_items is not None
            ):
            ...
            if issubclass(origin, frozenset) and (
                field_info.min_items is not None or field_info.max_items is not None
            ):
                used_constraints.update({"min_items", "max_items"})
                return confrozenset(
        ...
        constraint_func: Callable[..., type] | None = None
        if isinstance(type_, type):
            if issubclass(type_, (SecretStr, SecretBytes)):
                ...
                numeric_type = next(
                    t for t in numeric_types if issubclass(type_, t)
                )  # pragma: no branch
        ...
        if attrs:
            ...
            kwargs = {
                ...
                for attr_name, attr in (
                    (attr_name, getattr(field_info, attr_name)) for attr_name in attrs
                )
                if attr is not None
            }
            if kwargs:
