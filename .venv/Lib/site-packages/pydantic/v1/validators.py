...
def number_size_validator(v: "Number", field: "ModelField") -> "Number":
    ...
    if field_type.ge is not None and not v >= field_type.ge:
...
class IfConfig:
    def __init__(
        self, validator: AnyCallable, *config_attr_names: str, ignored_value: Any = False
    ) -> None:
...
def find_validators(  # noqa: C901 (ignore complexity)
    ...
    if config.arbitrary_types_allowed:
        ...
        if hasattr(type_, "__pydantic_core_schema__"):
            warn(
                f"Mixing V1 and V2 models is not supported. `{type_.__name__}` is a V2 model.",
                UserWarning,
            )
        raise RuntimeError(
