...
if TYPE_CHECKING:
    ...
    LocStr = Union[tuple[int | str, ...], str]
...
class FieldInfo(Representation):
    ...
    def __init__(self, default: Any = Undefined, **kwargs: Any) -> None:
        ...
        self.alias_priority = kwargs.pop(
            "alias_priority", 2 if self.alias is not None else None
        )
...
def Field(
    ...
    *,
    ...
    exclude: Union["AbstractSetIntStr", "MappingIntStrAny", Any] | None = None,
    include: Union["AbstractSetIntStr", "MappingIntStrAny", Any] | None = None,
...
class ModelField(Representation):
    ...
    def __init__(
        ...
        *,
        name: str,
        type_: type[Any],
        class_validators: dict[str, Validator] | None,
        ...
        self.default_factory: NoArgAnyCallable | None = default_factory
        self.required: BoolUndefined = required
        self.final: bool = final
        self.model_config = model_config
        self.field_info: FieldInfo = field_info or FieldInfo(default)
        self.discriminator_key: str | None = self.field_info.discriminator
        self.discriminator_alias: str | None = self.discriminator_key

        self.allow_none: bool = False
        self.validate_always: bool = False
        self.sub_fields: list[ModelField] | None = None
        self.sub_fields_mapping: dict[str, ModelField] | None = (
            None  # used for discriminated union
        )
        self.key_field: ModelField | None = None
        self.validators: ValidatorsList = []
        self.pre_validators: ValidatorsList | None = None
        self.post_validators: ValidatorsList | None = None
    ...
    def infer(
        ...
        *,
        ...
        annotation = get_annotation_from_field_info(
            annotation, field_info, name, config.validate_assignment
        )
    ...
    def set_config(self, config: type["BaseConfig"]) -> None:
        ...
        if new_include is not None:
            self.field_info.include = ValueItems.merge(
                self.field_info.include, new_include, intersect=True
            )
    ...
    def _type_analysis(self) -> None:  # noqa: C901 (ignore complexity)
        ...
        if self.type_ is Any or self.type_ is object:
            ...
            self.allow_none = True
            return
        if self.type_ is Pattern or self.type_ is re.Pattern:
            # python 3.7 only, Pattern is a typing object but without sub fields
            return
        if is_literal_type(self.type_) or is_typeddict(self.type_):
            return
    ...
    def prepare_discriminated_union_sub_fields(self) -> None:
        ...
        self.discriminator_alias = get_unique_discriminator_alias(
            all_aliases, self.discriminator_key
        )

    def _create_sub_type(
        self, type_: type[Any], name: str, *, for_keys: bool = False
    ) -> "ModelField":
        if for_keys:
    ...
    def populate_validators(self) -> None:
        ...
        if class_validators_:
            self.pre_validators += prep_validators(
                v.func for v in class_validators_ if not v.each_item and v.pre
            )
            self.post_validators += prep_validators(
                v.func for v in class_validators_ if not v.each_item and not v.pre
            )
    ...
    def _get_mapping_value(
        ...
        if original_cls == dict or original_cls == dict:
            return converted
        if original_cls in {defaultdict, defaultdict}:
        ...
        try:
            ...
            raise RuntimeError(
                f"Could not convert dictionary to {original_cls.__name__!r}"
            ) from None
    ...
    def _validate_discriminated_union(
        ...
        try:
            ...
            return v, ErrorWrapper(
                MissingDiscriminator(discriminator_key=self.discriminator_key), loc
            )
        except TypeError:
            try:
        ...
        try:
            ...
            if not isinstance(loc, tuple):
                loc = (loc,)
            return sub_field.validate(
                v, values, loc=(*loc, display_as_type(sub_field.type_)), cls=cls
            )
    ...
    def __repr_args__(self) -> "ReprArgs":
        ...
        if not self.required:
            if self.default_factory is not None:
                args.append(
                    ("default_factory", f"<function {self.default_factory.__name__}>")
                )
            else:
...
class ModelPrivateAttr(Representation):
    ...
    def __init__(
        self, default: Any = Undefined, *, default_factory: NoArgAnyCallable | None = None
    ) -> None:
        self.default = default
