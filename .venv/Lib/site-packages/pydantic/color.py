...
def parse_str(value: str) -> RGBA:
    ...
    * `rgba(<r>, <g>, <b>, <a>)`
    ...
    raise PydanticCustomError(
        "color_error", "value is not a valid color: string not recognised as a valid color"
    )


def ints_to_rgba(
    r: int | str, g: int | str, b: int | str, alpha: float | None = None
) -> RGBA:
...
def parse_color_value(value: int | str, max_val: int = 255) -> float:
    ...
    try:
        ...
        raise PydanticCustomError(
            "color_error", "value is not a valid color: color values must be a valid number"
        )
    if 0 <= color <= max_val:
...
def parse_float_alpha(value: None | str | float) -> float | None:
    ...
    try:
        ...
        raise PydanticCustomError(
            "color_error", "value is not a valid color: alpha values must be a valid float"
        )
...
def parse_hsl(
    h: str, h_units: str, sat: str, light: str, alpha: float | None = None
) -> RGBA:
