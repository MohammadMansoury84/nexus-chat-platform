...
class PydanticPlugin(Plugin):
    ...
    def _pydantic_create_model_callback(self, ctx: DynamicClassDefContext) -> None:
        ...
        for arg_name, arg_expr in zip(ctx.call.arg_names, ctx.call.args):
            if (
                arg_name == "__base__"
                and isinstance(arg_expr, RefExpr)
                and arg_expr.node is not None
            ):
                if isinstance(arg_expr.node, TypeInfo):
                    base_fullname = arg_expr.node.fullname
                elif isinstance(arg_expr.node, Var) and isinstance(
                    arg_expr.node.type, Instance
                ):
...
class PydanticModelTransformer:
        ...
        for info in reversed(
            ...
            for name, data in info.metadata[METADATA_KEY]["fields"].items():
                ...
                if (
                    sym_node
                    and sym_node.node
                    and not isinstance(sym_node.node, (Var, PlaceholderNode))
                ):
                    self._api.fail(
    ...
    def add_model_construct_method(
        ...
        args = (
            args + [fields_set_argument] if is_root_model else [fields_set_argument] + args
        )
            ...
            for arg, name in zip(expr.args, expr.arg_names):
                ...
                if isinstance(arg, NameExpr):
                    if arg.fullname == "builtins.True":
                        return True
                    if arg.fullname == "builtins.False":
                        return False
                return None
        return None
