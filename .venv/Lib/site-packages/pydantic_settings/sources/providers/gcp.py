...
class GoogleSecretManagerMapping(Mapping[str, str | None]):
    ...
    def _select_case_insensitive_secret(
    ...
    def _secret_name_map(self) -> dict[str, str]:
        ...
        if not self._case_sensitive:
            for lower_name, candidates in normalized_groups.items():
                mapping[lower_name] = self._select_case_insensitive_secret(
    ...
    def _get_secret_value(
        self, gcp_secret_name: str, version: str = "latest"
    ) -> str | None:
        try:
...
class GoogleSecretManagerSettingsSource(EnvSettingsSource):
    ...
    def get_field_value(self, field: FieldInfo, field_name: str) -> tuple[Any, str, bool]:
        ...
        """
        secret_version = next(
            (m.version for m in field.metadata if isinstance(m, SecretVersion)), None
        )
        ...
        if secret_version and isinstance(self.env_vars, GoogleSecretManagerMapping):
            for field_key, env_name, value_is_complex in self._extract_field_info(
                ...
                if gcp_secret_name:
                    env_val = self.env_vars._get_secret_value(
                        gcp_secret_name, secret_version
                    )
