...
class _CliArg(BaseModel):
    ...
    def __init__(
        ...
        parser_map: defaultdict[
            str | FieldInfo, dict[int | str | type[BaseModel] | None, _CliArg]
        ],
        **values: Any,
    ) -> None:
        ...
        alias_path_dests = {
            f"{self.arg_prefix}{name}": index for name, index in self._alias_paths.items()
        }
        if self.subcommand_dest:
            for sub_model in self.sub_models:
                subcommand_alias = self.subcommand_alias(sub_model)
                parser_map[self.subcommand_dest][subcommand_alias] = self.model_copy(
                    update={"args": [], "kwargs": {}}
                )
                parser_map[self.subcommand_dest][sub_model] = parser_map[
                    self.subcommand_dest
                ][subcommand_alias]
                parser_map[self.field_info][subcommand_alias] = parser_map[
                    self.subcommand_dest
                ][subcommand_alias]
        ...
        for alias_path_dest, index in alias_path_dests.items():
            parser_map[alias_path_dest][index] = self.model_copy(
                update={"args": [], "kwargs": {}}
            )
    ...
    @classmethod
    def get_kebab_case(
        cls, name: str, kebab_case: bool | Literal["all", "no_enums"] | None
    ) -> str:
    ...
    def get_enum_names(
        ...
        if annotation and _lenient_issubclass(annotation, Enum):
            enum_names += tuple(
                cls.get_kebab_case(name, kebab_case == "all")
                for name in annotation.__members__.keys()
            )
        return enum_names

    def subcommand_alias(self, sub_model: type[BaseModel]) -> str:
        return self.get_kebab_case(
            sub_model.__name__ if len(self.sub_models) > 1 else self.preferred_alias,
            self.kebab_case,
        )
    ...
    def subcommand_dest(self) -> str | None:
        return (
            f"{self.arg_prefix}:subcommand"
            if _CliSubCommand in self.field_info.metadata
            else None
        )
    ...
    def is_parser_submodel(self) -> bool:
        return (
            not self.subcommand_dest and bool(self.sub_models) and not self.is_append_action
        )
    ...
    def is_no_decode(self) -> bool:
        return self.field_info is not None and (
            ...
            or (
                self.enable_decoding is False
                and ForceDecode not in self.field_info.metadata
            )
        )
...
class CliSettingsSource(EnvSettingsSource, Generic[T]):
    ...
    def __init__(
        ...
        if self.cli_prefix:
            if (
                cli_prefix.startswith(".")
                or cli_prefix.endswith(".")
                or not cli_prefix.replace(".", "").isidentifier()
            ):  # type: ignore
        ...
        if not self.cli_ignore_unknown_args and self._cli_unknown_args:
            ...
            if has_unknown:
                selected_accepts_unknown = any(
                    dest.rsplit(".", 1)[0] in last_selected_subcommand
        ...
        for field_name, val in list(parsed_args.items()):
            if isinstance(val, list):
                ...
                if (
                    ...
                    and (
                        snake_val
                        in cli_arg.get_enum_names(cli_arg.field_info.annotation, False)
                    )
                ):
                    if "_" in val:
                        raise ValueError(
                            f'Input should be kebab-case "{val.replace("_", "-")}", not "{val}"'
                        )
    ...
    def _is_nested_alias_path_only_workaround(
        ...
        if arg.is_alias_path_only and arg.arg_prefix.endswith("."):
            ...
            nested_val = (
                f'"{arg.preferred_alias}": {self._merge_parsed_list(val, field_name)}'
            )
    ...
    def _get_merge_parsed_list_types(
        ...
        if (
            ...
            inferred_type = (
                list
                if parsed_list and (len(parsed_list) > 1 or parsed_list[0].startswith("["))
                else str
            )
    ...
    def _merged_list_to_str(self, merged_list: list[str], field_name: str) -> str:
        ...
        try:
            list_adapter: Any = TypeAdapter(
                next(iter(cli_arg_map.values())).field_info.annotation
            )
        ...
        for index, item in enumerate(merged_list):
            ...
            if is_use_decode:
                ...
                try:
                    unquoted_item = (
                        item[1:-1] if item.startswith('"') and item.endswith('"') else item
                    )
                    float(unquoted_item)
        ...
        for field_name, field_info in _get_model_fields(model).items():
            if _CliSubCommand in field_info.metadata:
                ...
                for field_type in field_types:
                    if not (
                        is_model_class(field_type) or is_pydantic_dataclass(field_type)
                    ):
                        raise SettingsError(
    ...
    def _connect_group_method(
        ...
        def add_group_method(parser: Any, **kwargs: Any) -> Any:
            ...
            main_group_kwargs = {
                arg: kwargs.pop(arg) for arg in ["title", "description"] if arg in kwargs
            }
    ...
    def _connect_root_parser(
        ...
        if (
            self._cli_unknown_args
            and not self.cli_ignore_unknown_args
            and _is_default_parse_args
        ):
            self._parse_args = self._connect_parser_method(
            ...
            if not self.cli_prefix:
                for field_name, field_info in _get_model_fields(self.settings_cls).items():
                    alias_names, *_ = _get_alias_names(
                        field_name, field_info, case_sensitive=self.case_sensitive
                    )
        ...
        for field_name, field_info in self._sort_arg_fields(model):
            ...
            if arg.subcommand_dest:
                for sub_model in arg.sub_models:
                    ...
                    subcommand_arg.kwargs["help"] = (
                        None if len(arg.sub_models) > 1 else field_info.description
                    )
                    ...
                    subcommand_arg.parser = self._add_parser(
                        subparsers, *subcommand_arg.args, **subcommand_arg.kwargs
                    )
                    self._add_parser_args(
                ...
                non_recursive_sub_models = [
                    m for m in arg.sub_models if m not in model_path
                ]
                if (
                    ...
                    if arg.kwargs.get("action") == "store_false":
                        flag_prefix += "no-"
                    arg.args = [
                        f"{flag_prefix[: 1 if len(name) == 1 else None]}{name}"
    ...
    def _convert_bool_flag(
        ...
        if kwargs["metavar"] == "bool":
            meta_bool_flags = [
                ...
                and issubclass(meta, _CliImplicitFlag | _CliExplicitFlag)
            ]
            ...
            if meta_bool_flags:
                ...
                if bool_flag is _CliImplicitFlag:
                    bool_flag = (
                        _CliToggleFlag
                        if self.cli_implicit_flags == "toggle"
                        and isinstance(field_info.default, bool)
                        else _CliDualFlag
                    )
    ...
    def _convert_positional_arg(
        ...
        kwargs["metavar"] = _CliArg.get_kebab_case(
            preferred_alias.upper(), self.cli_kebab_case
        )
    ...
    def _get_arg_names(
        ...
        if discriminators is not None:
            ...
            if not is_last_discriminator:
                return []
            arg.kwargs["metavar"] = self._metavar_format(
                Literal[tuple(sorted(discriminators))]
            )
    ...
    def _add_parser_submodels(
        ...
        discriminator_vals: dict[str, set[Any]] = (
            {f"{arg_prefix}{preferred_alias}.{field_info.discriminator}": set()}
    ...
    def _add_parser_alias_paths(
        ...
        if alias_path_args:
            ...
            if group is not None:
                context = (
                    self._add_group(parser, **group) if isinstance(group, dict) else group
                )
    ...
    def _metavar_format_recurse(self, obj: Any) -> str:
        ...
        if _lenient_issubclass(obj, Enum):
            return self._metavar_format_choices(
                [
                    _CliArg.get_kebab_case(name, self.cli_kebab_case == "all")
                    for name in obj.__members__.keys()
                ]
            )
    ...
    def _help_format(
        ...
        if field_info.is_required() and model_default in (PydanticUndefined, None):
            ...
            if is_model_class(type(model_default)) or is_pydantic_dataclass(
                ...
                default = (
                    f"(default: {field_info.default if enum_name is None else enum_name})"
                )
            elif field_info.default_factory is not None:
                default = (
                    f"(default factory: {self._metavar_format(field_info.default_factory)})"
                )
    ...
    def _serialized_args(
        ...
        for field_name, field_info in _get_model_fields(
            type(model) if _is_submodel else self.settings_cls
        ).items():
            ...
            if arg.subcommand_dest:
                ...
                subcommand_args += self._flatten_serialized_args(
                    sub_args, positionals_first
                )
                continue
            if is_model_class(type(model_default)) or is_pydantic_dataclass(
                type(model_default)
            ...
            value: str | list[Any] | dict[str, Any] = (
                json.dumps(model_default)
                if isinstance(model_default, (dict, list, set))
                else str(model_default)
            )
            ...
            if arg.is_alias_path_only:
                ...
                value = self._update_alias_path_only_default(
                    arg_name, value, field_info, alias_path_only_defaults
                )

            if _CliPositionalArg in field_info.metadata:
                for value in (
                    model_default if isinstance(model_default, list) else [model_default]
                ):
                    value = (
                        json.dumps(value)
            ...
            if (
                arg.kwargs.get("action") == BooleanOptionalAction
                and model_default is False
                and flag_chars == "--"
            ):
                flag_chars += "no-"

            for value in self._coerce_value_styles(
                ...
                if arg.kwargs.get("action") not in (
                    BooleanOptionalAction,
                    "store_true",
                    "store_false",
                ):
