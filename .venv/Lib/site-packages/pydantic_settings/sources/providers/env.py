...
class EnvSettingsSource(PydanticBaseEnvSettingsSource):
    ...
    def _load_env_vars(self) -> Mapping[str, str | None]:
        return parse_env_vars(
            os.environ, self.case_sensitive, self.env_ignore_empty, self.env_parse_none_str
        )
            ...
            for field_name, f in fields.items():
                for _, env_name, _ in self._extract_field_info(f, field_name):
                    if case_sensitive is None or case_sensitive:
                        ...
                        field_name.lower() == key.lower() or env_name.lower() == key.lower()
                    ):
                        return f
        return None
            ...
            *keys, last_key = env_name_without_prefix.split(
            ...
            if (target_field or is_dict) and env_val:
                ...
                if is_complex:
                    try:
                        field_info = (
                            target_field if isinstance(target_field, FieldInfo) else None
                        )
        ...
        try:
            ...
            if should_coerce and isinstance(value, str) and isinstance(field, FieldInfo):
                if value == self.env_parse_none_str:
                    return value
                if not _annotation_contains_types(
                    field.annotation, (Json,), is_instance=True
                ):
                    try:
