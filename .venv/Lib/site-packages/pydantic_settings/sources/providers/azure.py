        ...
        if self._snake_case_conversion:
            ...
            for name in secret_names:
                if name.startswith(self._env_prefix):
                    name_map[
                        f"{self._env_prefix}{to_snake(name[len(self._env_prefix) :])}"
                    ] = name
                else:
    ...
    def __getitem__(self, key: str) -> str | None:
        ...
        if new_key not in self._loaded_secrets:
            if new_key in self._secret_map:
                self._loaded_secrets[new_key] = self._secret_client.get_secret(
                    self._secret_map[new_key]
                ).value
            else:
                raise KeyError(key)
...
class AzureKeyVaultSettingsSource(EnvSettingsSource):
    ...
    def _extract_field_info(
        ...
        if self._snake_case_conversion:
            field_info = list(
                (x[0], x[1], x[2]) for x in super()._extract_field_info(field, field_name)
            )
            return field_info
