...
class DotEnvSettingsSource(EnvSettingsSource):
    ...
    def __init__(
        ...
        self.dotenv_filtering = (
            dotenv_filtering
            if dotenv_filtering is not None
            else settings_cls.model_config.get("dotenv_filtering")
        )
        super().__init__(
            settings_cls,
            case_sensitive,
    ...
    def __call__(self) -> dict[str, Any]:  # noqa: C901
            ...
            for env_name, env_value in self.env_vars.items():
                if env_name.startswith(prefix):
                    ...
                    if (
                        ...
                        and normalized_env_name.partition(self.env_nested_delimiter)[0]
                        in data
                    ):
                        continue
        ...
        for env_name, env_value in self.env_vars.items():
            if (
                not env_value
                or env_name in data
                or (self.env_prefix and env_name in self.settings_cls.model_fields)
            ):
                continue
            env_used = False
