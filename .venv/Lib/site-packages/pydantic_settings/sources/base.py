...
def get_subcommand(
    ...
    if is_required:
        ...
        err = (
            SystemExit(error_message) if cli_exit_on_error else SettingsError(error_message)
        )
...
class ConfigFileSourceMixin(ABC):
    def _read_files(
        self, files: PathType | None, deep_merge: bool = False
    ) -> dict[str, Any]:
        if files is None:
            return {}
    ...
    def __init__(
        ...
        for field_name, field_info in settings_cls.model_fields.items():
            ...
            if init_kwarg_name:
                ...
                provided_key = next(
                    (alias for alias in alias_names if alias in init_kwarg_names), None
                )
...
class PydanticBaseEnvSettingsSource(PydanticBaseSettingsSource):
    def __init__(
        ...
        self.env_prefix_target = (
            env_prefix_target
            if env_prefix_target is not None
            else self.config.get("env_prefix_target", "variable")
        )
    ...
    def _extract_field_info(
        ...
        if v_alias:
            env_prefix = (
                self.env_prefix if self.env_prefix_target in ("alias", "all") else ""
            )
            if isinstance(v_alias, list):  # AliasChoices, AliasPath
                for alias in v_alias:
                    if isinstance(alias, str):  # AliasPath
                        field_info.append(
                            (
                                alias,
                                self._apply_case_sensitive(env_prefix + alias),
                ...
                field_info.append(
                    (v_alias, self._apply_case_sensitive(env_prefix + v_alias), False)
                )

        if (
            not v_alias
            or self.config.get("populate_by_name", False)
            or self.config.get("validate_by_name", False)
        ):
            annotation = _strip_annotated(_resolve_type_alias(field.annotation))
            env_prefix = (
                self.env_prefix if self.env_prefix_target in ("variable", "all") else ""
            )
            if is_union_origin(get_origin(annotation)) and _union_is_complex(
                annotation, field.metadata
            ):
                field_info.append(
                    (field_name, self._apply_case_sensitive(env_prefix + field_name), True)
                )
            else:
                field_info.append(
                    (field_name, self._apply_case_sensitive(env_prefix + field_name), False)
                )
    ...
    def _get_resolved_field_value(
        ...
        if not (
            ...
            or (
                (
                    self.config.get("populate_by_name", False)
                    or self.config.get("validate_by_name", False)
                )
