...
class BaseSettings(BaseModel):
    ...
    def __init__(
        ...
        **values: Any,
        ...
        super().__init__(
            **__pydantic_self__.__class__._settings_build_values(sources, init_kwargs)
        )
    ...
    def _settings_init_sources(
        ...
        case_sensitive = (
            ...
            else cls.model_config.get("case_sensitive")
        )
        env_prefix = (
            _env_prefix if _env_prefix is not None else cls.model_config.get("env_prefix")
        )
        env_prefix_target = (
            _env_prefix_target
            if _env_prefix_target is not None
            else cls.model_config.get("env_prefix_target")
        )
        ...
        env_file = (
            ...
            else cls.model_config.get("env_file")
        )
        env_file_encoding = (
            _env_file_encoding
            if _env_file_encoding is not None
            else cls.model_config.get("env_file_encoding")
        )
        env_ignore_empty = (
            _env_ignore_empty
            if _env_ignore_empty is not None
            else cls.model_config.get("env_ignore_empty")
        )
        env_nested_delimiter = (
            _env_nested_delimiter
            if _env_nested_delimiter is not None
            else cls.model_config.get("env_nested_delimiter")
        )
        env_nested_max_split = (
            _env_nested_max_split
            if _env_nested_max_split is not None
            else cls.model_config.get("env_nested_max_split")
        )
        env_parse_none_str = (
            _env_parse_none_str
            if _env_parse_none_str is not None
            else cls.model_config.get("env_parse_none_str")
        )
        env_parse_enums = (
            _env_parse_enums
            if _env_parse_enums is not None
            else cls.model_config.get("env_parse_enums")
        )

        cli_prog_name = (
            _cli_prog_name
            if _cli_prog_name is not None
            else cls.model_config.get("cli_prog_name")
        )
        cli_parse_args = (
            _cli_parse_args
            if _cli_parse_args is not None
            else cls.model_config.get("cli_parse_args")
        )
        cli_settings_source = (
            _cli_settings_source
            if _cli_settings_source is not None
            else cls.model_config.get("cli_settings_source")
        )
        cli_parse_none_str = (
            _cli_parse_none_str
            if _cli_parse_none_str is not None
            else cls.model_config.get("cli_parse_none_str")
        )
        cli_parse_none_str = (
        ...
        cli_hide_none_type = (
            ...
            else cls.model_config.get("cli_hide_none_type")
        )
        cli_avoid_json = (
            _cli_avoid_json
            if _cli_avoid_json is not None
            else cls.model_config.get("cli_avoid_json")
        )
        cli_enforce_required = (
            _cli_enforce_required
            if _cli_enforce_required is not None
            else cls.model_config.get("cli_enforce_required")
        )
        ...
        cli_exit_on_error = (
            ...
            else cls.model_config.get("cli_exit_on_error")
        )
        cli_prefix = (
            _cli_prefix if _cli_prefix is not None else cls.model_config.get("cli_prefix")
        )
        cli_flag_prefix_char = (
            _cli_flag_prefix_char
            if _cli_flag_prefix_char is not None
            else cls.model_config.get("cli_flag_prefix_char")
        )
        cli_implicit_flags = (
            _cli_implicit_flags
            if _cli_implicit_flags is not None
            else cls.model_config.get("cli_implicit_flags")
        )
        ...
        cli_kebab_case = (
            ...
            else cls.model_config.get("cli_kebab_case")
        )
        cli_shortcuts = (
            _cli_shortcuts
            if _cli_shortcuts is not None
            else cls.model_config.get("cli_shortcuts")
        )

        secrets_dir = (
            _secrets_dir
            if _secrets_dir is not None
            else cls.model_config.get("secrets_dir")
        )
            ...
            for field_name, field_info in settings_cls.model_fields.items():
                ...
                if init_kwarg_name and state_kwarg_name:
                    ...
                    if source_key is None:
                        ...
                        source_key = (
                            field_name
                            if field_name in state_kwarg_name
                            else next(iter(state_kwarg_name))
                        )
    ...
    def _settings_warn_unused_config_keys(
        sources: tuple[object, ...], model_config: SettingsConfigDict
    ) -> None:
...
class CliApp:
    ...
    def run(
        ...
        **model_init_data: Any,
        ...
        if not issubclass(model_cls, BaseSettings):
            ...
            model = base_settings_cls(
                **base_settings_cls._settings_build_values(sources, init_kwargs)
            )
            model_init_data = {}
        ...
        CliApp._subcommand_stack[id(command)] = (
            cli_settings_source,
            cli_settings_source.root_parser,
            subcommand_dest,
        )
        try:
            data_model = CliApp._run_cli_cmd(
                command, cli_cmd_method_name, is_required=False
            )
        finally:
    ...
    def run_subcommand(
        ...
        if id(model) in CliApp._subcommand_stack:
            cli_settings_source, parser, subcommand_dest = CliApp._subcommand_stack[
                id(model)
            ]
        else:
        ...
        cli_exit_on_error = (
            cli_settings_source.cli_exit_on_error
        ...
        if errors:
            ...
            if (
                ...
                and cli_settings_source._format_help is not None
            ):
        ...
        CliApp._subcommand_stack[id(subcommand)] = (
            cli_settings_source,
            subcommand_parser,
            subcommand_dest,
        )
        try:
            data_model = CliApp._run_cli_cmd(
                subcommand, cli_cmd_method_name, is_required=True
            )
        finally:
    ...
    def serialize(
        ...
        return CliSettingsSource._flatten_serialized_args(
            serialized_args, positionals_first
        )
        ...
        if cli_settings_source is None:
            if not isinstance(model, type) and id(model) in CliApp._subcommand_stack:
                ...
                cli_settings_source = CliSettingsSource(
                    CliApp._get_base_settings_cls(model_cls)
                )
        help_message = cli_settings_source._format_help(cli_settings_source.root_parser)
        return (
            help_message
            if not strip_ansi_color
            else CliApp._ansi_color.sub("", help_message)
        )
