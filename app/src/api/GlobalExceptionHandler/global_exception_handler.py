...
class GlobalExceptionHandler:
    ...
    def register_handlers(self):
        ...
        # )
        self.app.add_exception_handler(
            AuthorizationError,self.handle_authorization_error
        )
        self.app.add_exception_handler(
        ...
        self.app.add_exception_handler(
            EmptyDataException,self.handle_empty_data_exception
        )
        self.app.add_exception_handler(
        ...
        self.app.add_exception_handler(
        ResponseError,self.handle_response_error
        )
        self.app.add_exception_handler(
        UserAlreadyInGroupError,self.handle_user_already_in_group_error
        )    
        self.app.add_exception_handler(
        UserNotFoundError,self.handle_user_not_found_error
        )    
        self.app.add_exception_handler(
    ...
    async def handle_duplicate_email_error(self,request: Request , exc:DuplicateEmailError):
        return JSONResponse(
    ...
    async def handle_duplicdate_username_error(self,request: Request , exc:DuplicateUsernameError):
        return JSONResponse(
            status_code=status.HTTP_409_CONFLICT,
            content={"detail": str(exc)},
        )
    ...
    async def handle_invalid_credentials_error(self,request: Request , exc:InvalidCredentialsError):
        return JSONResponse(
            status_code=status.HTTP_401_UNAUTHORIZED,
            content={"detail": str(exc)},
        )
    ...
    async def handle_private_chat_not_found_error(self,request: Request , exc:PrivateChatNotFoundError):
        return JSONResponse(
            status_code=status.HTTP_404_NOT_FOUND,
            content={"detail": str(exc)},
        )
    ...
    async def handle_user_already_in_group_error(self,request: Request , exc:UserAlreadyInGroupError):
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={"detail": str(exc)},
        )
    ...
    async def handle_user_not_in_group_error(self,request: Request , exc:UserNotInGroupError):
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={"detail": str(exc)},
        )
