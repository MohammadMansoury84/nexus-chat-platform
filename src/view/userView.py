...
class UserView:
    ...
    def signup(self):
        ...
        if user_id is not None:
            print("User created successfully.login first")
    ...
    def send_message(self) -> None:
        ...
        while True:
            ...
            if content.lower() == "change":
                new_receiver = self._select_user(
                    users=users, title="Who do you want to message?"
                )
                ...
                if new_receiver is not None:
                    ...
                    chat_history = self._message_controller.get_chat(
                        self.current_user.id, receiver.id
                    )
                    if chat_history:
                        print("\n[Previous Messages]")
                        self._print_chat(chat_history)
            ...
            try:
                self._message_controller.send_message(
                    sender_id=self.current_user.id, receiver_id=receiver.id, content=content
                )
    ...
    def show_private_chat(self) -> None:
        ...
        chat = self._message_controller.get_chat(
            user1_id=self.current_user.id, user2_id=other_user.id
        )
    ...
    def create_group(self) -> None:
        ...
        group_id = self._group_controller.create_group(
            name=name, creator_id=self.current_user.id
        )
    ...
    def add_user_to_group(self) -> None:
        ...
        result = self._group_controller.add_user_to_group(
            group_id=group.id, creator_id=self.current_user.id, user_id=user.id
        )
